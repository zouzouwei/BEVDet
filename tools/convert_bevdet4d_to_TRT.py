...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            feat_prev, inputs_ = model.extract_img_feat(
        ...
        feat_prev=feat_prev.view(1,640,128,128)
        
        with torch.no_grad():
            torch.onnx.export(
                model,
                (inputs[0].float().contiguous(),feat_prev.float().contiguous(), metas[1].int().contiguous(),
                ...
                opset_version=11,
                input_names=[
                    'img', 'feat_bev_list','ranks_depth', 'ranks_feat', 'ranks_bev',
                    'interval_starts', 'interval_lengths'
                ],
                output_names=[f'output_{j}' for j in range(37)])
        break
    # check onnx model
